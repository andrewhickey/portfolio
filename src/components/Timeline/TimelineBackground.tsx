...
function TimelineBackground({
  ...
  centerLinePath.moveTo(horizontalCenter, 0)
  centerLinePath.lineTo(horizontalCenter, height)
  ...
  return (
    ...
    >
      <path stroke="teal" strokeWidth={10} d={centerLinePath.toString()} />
      {Object.entries(eventDimensions).map(([id, dimensions]) => {
        ...
        if (adjustedLeft + 1 >= horizontalCenter) {
          ...
          borderBottomPath.lineTo(adjustedLeft, adjustedTop + dimensions.height)
